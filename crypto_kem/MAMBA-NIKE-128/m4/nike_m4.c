...
static void nike_kdf(unsigned char *out, const unsigned char *nu, const unsigned char *pk, const unsigned char *m1)
{
  ...
  static unsigned char input[NIKE_KDF_INPUTBYTES];
  unsigned int off = 0;
  int i;
...
void nike_keygen(unsigned char *send, poly *sk)
{
  ...
  randombytes(seed, NIKE_SEEDBYTES);
...
void nike_shareda(unsigned char *sharedkey, const poly *sk, const unsigned char *pk, const unsigned char *received)
{
  poly v,bp, c, du, dv;
  unsigned char mu[NIKE_SEEDBYTES];
#ifndef STATISTICAL_TEST
