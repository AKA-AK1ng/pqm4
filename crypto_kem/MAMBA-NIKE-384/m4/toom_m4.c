...
static int64_t toom4_workspace[TOOM4_WORKWORDS];
static size_t toom4_workspace_top;

static size_t toom4_mark(void)
{
  return toom4_workspace_top;
}

static void toom4_release(size_t mark)
{
  toom4_workspace_top = mark;
}

static int64_t *toom4_alloc(int n)
{
  size_t count = (size_t)n;
  int64_t *r;

  if(toom4_workspace_top + count > TOOM4_WORKWORDS)
    return NULL;

  r = toom4_workspace + toom4_workspace_top;
  toom4_workspace_top += count;
...
static void toom4_mul_rec(int64_t *r, const int64_t *a, const int64_t *b, int n)
{
  ...
  int64_t *t1 = toom4_alloc(lp);
  int64_t *t2 = toom4_alloc(lp);
  int64_t *t3 = toom4_alloc(lp);
  int64_t *t4 = toom4_alloc(lp);
  int64_t *fh = toom4_alloc(lp);
  int64_t *rd = toom4_alloc(lp);
  int64_t *re = toom4_alloc(lp);
  int64_t *tt = toom4_alloc(lp);
...
{
  toom4_workspace_top = 0;
  toom4_mul_rec(r, a, b, n);
}
