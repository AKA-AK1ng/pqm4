...
int crypto_kem_keypair(uint8_t *pk, uint8_t *sk)
{
    ...
    frodo_quantize_dithered_local(B, B, PARAMS_N * PARAMS_NBAR, pk_seedA, BYTES_SEED_A, DITHER_DOMAIN_PK, PARAMS_PK_LOGP);
...
int crypto_kem_enc(uint8_t *ct, uint8_t *ss, const uint8_t *pk)
{
    ...
    frodo_quantize_dithered_local(Bp_split, Bp_raw, PARAMS_N * PARAMS_NBAR, salt, BYTES_SALT, DITHER_DOMAIN_U, PARAMS_U_LOGP);
    ...
    frodo_reconstruct_dithered_local(B_norm, B_split, PARAMS_N * PARAMS_NBAR, pk_seedA, BYTES_SEED_A, DITHER_DOMAIN_PK, PARAMS_PK_LOGP);
...
int crypto_kem_dec(uint8_t *ss, const uint8_t *ct, const uint8_t *sk)
{
    ...
    frodo_reconstruct_dithered_local(Bp_norm, Bp_split, PARAMS_N * PARAMS_NBAR, ct_salt, BYTES_SALT, DITHER_DOMAIN_U, PARAMS_U_LOGP);
    frodo_reconstruct_dithered_local(C_norm, C_split, PARAMS_NBAR * PARAMS_NBAR, ct_salt, BYTES_SALT, DITHER_DOMAIN_V, PARAMS_V_LOGP);
    ...
    for (size_t i = 0; i < PARAMS_N * PARAMS_NBAR; i++) {
        Bp_check[i] &= frodo_q_mask_local();
    }
    frodo_quantize_dithered_local(Bp_check, Bp_check, PARAMS_N * PARAMS_NBAR, ct_salt, BYTES_SALT, DITHER_DOMAIN_U, PARAMS_U_LOGP);
    ...
    frodo_quantize_dithered_local(C_check, C_check, PARAMS_NBAR * PARAMS_NBAR, ct_salt, BYTES_SALT, DITHER_DOMAIN_V, PARAMS_V_LOGP);

    memcpy(Fin_ct, ct, CRYPTO_CIPHERTEXTBYTES);
    {
        int8_t selector = ct_verify(Bp_split, Bp_check, PARAMS_N * PARAMS_NBAR) |
                          ct_verify(C_split, C_check, PARAMS_NBAR * PARAMS_NBAR);
    ...
    clear_bytes((uint8_t *)&state, sizeof(state));
    clear_bytes((uint8_t *)B_split, sizeof(B_split));
    clear_bytes((uint8_t *)B_norm, sizeof(B_norm));
    clear_bytes((uint8_t *)Bp_split, sizeof(Bp_split));
