...
int crypto_kem_enc(uint8_t *ct, uint8_t *ss, const uint8_t *pk)
{
    ...
    /* Bp_raw is no longer needed after packing; reuse its prefix for B. */
    frodo_unpack(Bp_raw, PARAMS_N * PARAMS_NBAR_R, pk_b, PK_PACKED_BYTES, PARAMS_PK_LOGP);
    frodo_reconstruct_dithered_local(Bp_raw, Bp_raw, PARAMS_N * PARAMS_NBAR_R, pk_seedA, BYTES_SEED_A, DITHER_DOMAIN_PK, PARAMS_PK_LOGP);
    memset(V_raw, 0, sizeof(V_raw));
    frodo_mul_add_sb_plus_e(V_raw, Bp_raw, Sp);
...
int crypto_kem_dec(uint8_t *ss, const uint8_t *ct, const uint8_t *sk)
{
    ...
    shake128_inc_absorb(&state, shake_input_seedSE, 1 + BYTES_SEED_SE);
    shake128_inc_finalize(&state);
    shake128_inc_squeeze((uint8_t *)Sp, PARAMS_N * PARAMS_NBAR_R * sizeof(uint16_t), &state);
    for (size_t i = 0; i < PARAMS_N * PARAMS_NBAR_R; i++) {
        Sp[i] = LE_TO_UINT16(Sp[i]);
    }
    frost_sample_n(Sp, PARAMS_N * PARAMS_NBAR_R);
    ...
    /* U has been checked; reuse Bp_norm's prefix for reconstructed public B. */
    memset(C_norm, 0, sizeof(C_norm));
    frodo_unpack(Bp_norm, PARAMS_N * PARAMS_NBAR_R, pk_b, PK_PACKED_BYTES, PARAMS_PK_LOGP);
    frodo_reconstruct_dithered_local(Bp_norm, Bp_norm, PARAMS_N * PARAMS_NBAR_R, pk_seedA, BYTES_SEED_A, DITHER_DOMAIN_PK, PARAMS_PK_LOGP);
    frodo_mul_add_sb_plus_e(C_norm, Bp_norm, Sp);
