...
{ // FrodoKEM's key encapsulation with public dithered quantization
    ...
    uint16_t C[PARAMS_NBAR*PARAMS_NBAR] = {0};                   // contains secret data
    ...
    if (frodo_quantize_dithered_local(Bp, Bp, PARAMS_N*PARAMS_NBAR, salt, BYTES_SALT, DITHER_DOMAIN_U, PARAMS_U_LOGP) != 0) {
        ret = 1;
        goto cleanup;
    }
    frodo_pack(ct_c1, CT_C1_PACKED_BYTES, Bp, PARAMS_N*PARAMS_NBAR, PARAMS_U_LOGP);

    frodo_unpack(B, PARAMS_N*PARAMS_NBAR, pk_b, PK_PACKED_BYTES, PARAMS_PK_LOGP);
    if (frodo_reconstruct_dithered_local(B, B, PARAMS_N*PARAMS_NBAR, pk_seedA, BYTES_SEED_A, DITHER_DOMAIN_PK, PARAMS_PK_LOGP) != 0) {
        ret = 1;
        goto cleanup;
    }
    ...
    frodo_add(C, V, C);
    if (frodo_quantize_dithered_local(C, C, PARAMS_NBAR*PARAMS_NBAR, salt, BYTES_SALT, DITHER_DOMAIN_V, PARAMS_V_LOGP) != 0) {
        ret = 1;
        goto cleanup;
    }
    frodo_pack(ct_c2, CT_C2_PACKED_BYTES, C, PARAMS_NBAR*PARAMS_NBAR, PARAMS_V_LOGP);
...
{ // FrodoKEM's key decapsulation with public dithered quantization
    ...
    uint16_t BBp[PARAMS_N*PARAMS_NBAR] = {0};                         // contains secret data
    ALIGN_HEADER(32) uint16_t Sp[PARAMS_N*PARAMS_NBAR] ALIGN_FOOTER(32) = {0};  // contains secret data
    ...
    frodo_unpack(Bp, PARAMS_N*PARAMS_NBAR, ct_c1, CT_C1_PACKED_BYTES, PARAMS_U_LOGP);
    frodo_unpack(C, PARAMS_NBAR*PARAMS_NBAR, ct_c2, CT_C2_PACKED_BYTES, PARAMS_V_LOGP);
    if (frodo_reconstruct_dithered_local(BBp, Bp, PARAMS_N*PARAMS_NBAR, salt, BYTES_SALT, DITHER_DOMAIN_U, PARAMS_U_LOGP) != 0 ||
        frodo_reconstruct_dithered_local(CC, C, PARAMS_NBAR*PARAMS_NBAR, salt, BYTES_SALT, DITHER_DOMAIN_V, PARAMS_V_LOGP) != 0) {
        ret = 1;
        goto cleanup;
    }
    frodo_mul_bs(W, BBp, S);
    frodo_sub(W, CC, W);
    ...
    frodo_mul_add_sa_plus_e(BBp, Sp, BBp, pk_seedA);
    for (size_t i = 0; i < PARAMS_N * PARAMS_NBAR; i++) {
        BBp[i] &= frodo_q_mask_local();
    }
    if (frodo_quantize_dithered_local(BBp, BBp, PARAMS_N*PARAMS_NBAR, salt, BYTES_SALT, DITHER_DOMAIN_U, PARAMS_U_LOGP) != 0) {
        ret = 1;
        goto cleanup;
    }
    ...
    frodo_mul_add_sb_plus_e(CC, B, Sp, CC);
    frodo_key_encode(W, (uint16_t*)muprime);
    frodo_add(CC, CC, W);
    if (frodo_quantize_dithered_local(CC, CC, PARAMS_NBAR*PARAMS_NBAR, salt, BYTES_SALT, DITHER_DOMAIN_V, PARAMS_V_LOGP) != 0) {
        ret = 1;
        goto cleanup;
    }
