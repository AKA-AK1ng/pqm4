...
static void viper_sample_secret_ntt(uint32_t s_ntt[VIPER_K][VIPER_N], unsigned char *skpke, const unsigned char seed[32]) {
  static const int16_t eta1_centered_lut[4] = {0, 1, -1, 0};
  unsigned char buf[VIPER_K * VIPER_N];
  shake256(buf, sizeof(buf), seed, 32);
  for (size_t i = 0; i < VIPER_K; i++) {
    int16_t centered[VIPER_N];
    for (size_t j = 0; j < VIPER_N; j++) {
      centered[j] = eta1_centered_lut[buf[i * VIPER_N + j] & 3u];
    }
    if (skpke != NULL) {
      viper_pack_secret12(skpke + i * VIPER_POLYBYTES_12, (const uint16_t *)centered);
    }
    poly_centered_ntt_m4(s_ntt[i], centered);
  }
}
...
void viper_pke_keypair(unsigned char *pk, unsigned char *skpke, const unsigned char rho[32], const unsigned char sseed[32]) {
  ...
  keypair_emit_ctx emit_ctx = {pk, rho};
  memcpy(pk, rho, 32);
  viper_sample_secret_ntt(s_ntt, skpke, sseed);
  matvec_stream_m4ntt_prepared(s_ntt, genpublic_expand_A_centered_cb, rho, emit_keypair_poly, &emit_ctx, 0);
}
...
void viper_pke_enc(unsigned char *ct, const unsigned char *pk, const unsigned char m[32], const unsigned char omega[64]) {
  ...
  uint32_t r_ntt[VIPER_K][VIPER_N];
  ...
  viper_sample_secret_ntt(r_ntt, NULL, omega);
  matTvec_dot_stream_m4ntt_prepared(t, r_ntt, genpublic_expand_A_centered_cb, rho, expand_public_poly_centered, &public_ctx, emit_ciphertext_u, &emit_ctx);
...
int viper_reencrypt_check(const unsigned char *ct, const unsigned char *pk, const unsigned char m[32], const unsigned char sigma[32], const unsigned char dither[VIPER_DITHER_BYTES]) {
  ...
  viper_sample_secret_ntt(r_ntt, NULL, omega);
  matTvec_dot_stream_m4ntt_prepared(t, r_ntt, genpublic_expand_A_centered_cb, rho, expand_public_poly_centered, &public_ctx, compare_ciphertext_u, &compare_ctx);
  viper_encode_add(acc, m, t);
