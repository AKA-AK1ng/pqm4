 ...
 * baseline polynomial multiplication support layer for the current implementation.
 */
...
#define N_RES (VIPER_N << 1)
#define N_SB (VIPER_N >> 2)
...
/* res += a*b */
static void basebackend_poly_mul_acc_internal(const uint16_t a[VIPER_N], const uint16_t b[VIPER_N], uint16_t res[VIPER_N])
{
	uint16_t c[2 * VIPER_N] = {0};
	int i;

	toom_cook_4way(a, b, c);

	/* reduction */
	for (i = VIPER_N; i < 2 * VIPER_N; i++)
	{
		res[i - VIPER_N] += (c[i - VIPER_N] - c[i]);
	}
}


static void poly_mul_basebackend_core(vpoly c, const vpoly a, const vpoly b)
{
  memset(c, 0, sizeof(vpoly));
  basebackend_poly_mul_acc_internal(a, b, c);
  for (size_t i = 0; i < VIPER_N; i++) c[i] &= VIPER_Q_MASK;
}

#ifdef VIPER_ARITH_AVX_WRAPPER
void poly_mul_basebackend_avx(vpoly c, const vpoly a, const vpoly b)
{
  poly_mul_basebackend_core(c, a, b);
}
...
void matvec_basebackend_avx(vpolyvec out, vpoly A[VIPER_K][VIPER_K], const vpolyvec s)
{
  ...
  for (size_t i = 0; i < VIPER_K; i++) {
    ...
    for (size_t j = 0; j < VIPER_K; j++) {
      poly_mul_basebackend_avx(t, A[i][j], s[j]);
...
void matTvec_basebackend_avx(vpolyvec out, vpoly A[VIPER_K][VIPER_K], const vpolyvec s)
{
  ...
  for (size_t i = 0; i < VIPER_K; i++) {
    memset(out[i], 0, sizeof(vpoly));
    for (size_t j = 0; j < VIPER_K; j++) {
      poly_mul_basebackend_avx(t, A[j][i], s[j]);
      for (size_t k = 0; k < VIPER_N; k++) out[i][k] = (uint16_t)((out[i][k] + t[k]) & VIPER_Q_MASK);
    }
  }
}
...
void dot_basebackend_avx(vpoly out, const vpolyvec a, const vpolyvec b)
{
  ...
  for (size_t i = 0; i < VIPER_K; i++) {
    poly_mul_basebackend_avx(t, a[i], b[i]);
...
void matvec_basebackend_ref(vpolyvec out, vpoly A[VIPER_K][VIPER_K], const vpolyvec s)
{
  ...
  for (size_t i = 0; i < VIPER_K; i++) {
    memset(out[i], 0, sizeof(vpoly));
    for (size_t j = 0; j < VIPER_K; j++) {
      poly_mul_basebackend_ref(t, A[i][j], s[j]);
      for (size_t k = 0; k < VIPER_N; k++) out[i][k] = (uint16_t)((out[i][k] + t[k]) & VIPER_Q_MASK);
    }
  }
}
...
void matTvec_basebackend_ref(vpolyvec out, vpoly A[VIPER_K][VIPER_K], const vpolyvec s)
{
  ...
  for (size_t i = 0; i < VIPER_K; i++) {
    memset(out[i], 0, sizeof(vpoly));
    for (size_t j = 0; j < VIPER_K; j++) {
      poly_mul_basebackend_ref(t, A[j][i], s[j]);
      for (size_t k = 0; k < VIPER_N; k++) out[i][k] = (uint16_t)((out[i][k] + t[k]) & VIPER_Q_MASK);
    }
  }
}
...
void dot_basebackend_ref(vpoly out, const vpolyvec a, const vpolyvec b)
{
  ...
  memset(out, 0, sizeof(vpoly));
  for (size_t i = 0; i < VIPER_K; i++) {
    poly_mul_basebackend_ref(t, a[i], b[i]);
    for (size_t k = 0; k < VIPER_N; k++) out[k] = (uint16_t)((out[k] + t[k]) & VIPER_Q_MASK);
  }
}
#endif
